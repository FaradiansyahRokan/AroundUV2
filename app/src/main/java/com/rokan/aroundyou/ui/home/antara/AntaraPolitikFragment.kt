...
import com.rokan.aroundyou.databinding.FragmentAntaraPolitikBinding
...
class AntaraPolitikFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        commonViewModel.getAntaraPolitikNews()
        commonViewModel.AntaraPolitikNews.observe(viewLifecycleOwner) { newsResponse ->
            val articles = newsResponse.data?.posts
            articles?.let {
                mAdapter.setData(it.filterNotNull())
                binding.rvAntaraPolitik.apply {
                    adapter = mAdapter
