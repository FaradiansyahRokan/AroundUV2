...
import com.rokan.aroundyou.databinding.FragmentAntaraTerbaruBinding
...
class AntaraTerbaruFragment : Fragment() {
    ...
    private lateinit var binding: FragmentAntaraTerbaruBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentAntaraTerbaruBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        commonViewModel.getAntaraTerbaruNews()
        commonViewModel.AntaraTerbaruNews.observe(viewLifecycleOwner) { newsResponse ->
            val articles = newsResponse.data?.posts
            articles?.let {
                mAdapter.setData(it.filterNotNull())
                binding.rvAntaraTerbaru.apply {
                    adapter = mAdapter
